...
function cvCreateMat(rows: cint; cols: cint; _type: cint): PCvMat; cdecl;
	{$IF Defined(MSWINDOWS)}external libopencvwrapper{$ELSE}external{$ENDIF} name 'USDX_cvCreateMat';
procedure cvReleaseMat(mat: PPCvMat); cdecl;
	{$IF Defined(MSWINDOWS)}external libopencvwrapper{$ELSE}external{$ENDIF} name 'USDX_cvReleaseMat';
procedure cvSetReal2D(arr: PCvMat; idx0: cint; idx1: cint; value: cdouble); cdecl;
	{$IF Defined(MSWINDOWS)}external libopencvwrapper{$ELSE}external{$ENDIF} name 'USDX_cvSetReal2D';
procedure cvSetZero(arr: PCvMat); cdecl;
	{$IF Defined(MSWINDOWS)}external libopencvwrapper{$ELSE}external{$ENDIF} name 'USDX_cvSetZero';
...
procedure cvGEMM(src1: PCvMat; src2: PCvMat; alpha: cdouble; src3: PCvMat; beta: cdouble; dst: PCvMat; tABC: cint = 0); cdecl;
	{$IF Defined(MSWINDOWS)}external libopencvwrapper{$ELSE}external{$ENDIF} name 'USDX_cvGEMM';
...
function cvInvert(A: PCvMat; B: PCvMat; method: cint = CV_LU): cdouble; cdecl;
	{$IF Defined(MSWINDOWS)}external libopencvwrapper{$ELSE}external{$ENDIF} name 'USDX_cvInvert';
