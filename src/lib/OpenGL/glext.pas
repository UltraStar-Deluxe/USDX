...
begin
  ...
  if not Assigned(glCopyTexSubImage3D) then Result := false;
  @glDrawRangeElements := SDL_GL_GetProcAddress('glDrawRangeElements');
  if not Assigned(glDrawRangeElements) then Result := false;
  @glTexImage3D := SDL_GL_GetProcAddress('glTexImage3D');
  if not Assigned(glTexImage3D) then Result := false;
  @glTexSubImage3D := SDL_GL_GetProcAddress('glTexSubImage3D');
  if not Assigned(glTexSubImage3D) then Result := false;
end;
